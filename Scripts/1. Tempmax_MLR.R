#Importar la base de datos: Vivtodas_diario_máxima.xlsx
...
Vivtodas_diario_máxima <- Vivtodas_diario_máxima %>%
...
Vivtodas_diario_máxima <- na.omit(Vivtodas_diario_máxima)
...
Viv_sinnormativa <- Vivtodas_diario_máxima %>%
  filter(dwell_numb %in% c(1, 2, 3, 4))

Viv_ct79 <- Vivtodas_diario_máxima %>%
  filter(dwell_numb %in% c(5, 6))

Viv_cte2006 <- Vivtodas_diario_máxima %>%
  filter(dwell_numb %in% c(7, 8, 9))

Viv_cte2019 <- Vivtodas_diario_máxima %>%
  filter(dwell_numb %in% c(10, 11, 12))


#MODELO PREDICTIVO de TEMPERATURA INTERIOR MÁXIMA (VARIABLE CONTINUA) 
